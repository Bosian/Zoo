...
/**
 ...
 ## 1.基本查詢
 您可以透過以下查詢方式查詢Data.Taipei的所有資料項目：
 https://data.taipei/opendata/datalist/apiAccess?scope=resourceMetadataSearch
 
 ## 2.查詢結果將會是符合條件的資料集清單，您可以在資料集清單中找到所有的資料項目。若您不指定任何查詢條件，則將會回傳Data.Taipei上的所有資料項目，我們非常不鼓勵您這麼做。
 若您要查詢包含某個關鍵字的資料項目，可以透過關鍵字查詢的方式進行。以臺北市文化快遞資訊為關鍵字，查詢方式如下：
 
 https://data.taipei/opendata/datalist/apiAccess?scope=resourceMetadataSearch&q=臺北市文化快遞資訊
 */
public struct GetZooWebAPI: HttpGet {
    
    public typealias TParameter = GetZooParameter
    public typealias TResult = GetZooModel
    public typealias TParser = GetZooParser
    
    public let urlString: String = "https://data.taipei/opendata/datalist/apiAccess"
...
/**
 # 將查詢結果分頁回傳
 ## 若符合條件的資料集太多，我們建議您使用每次回傳上限(limit)以及每次回傳位移筆數(offset)來調整您每次回傳的查詢結果。如下：

 https://data.taipei/opendata/datalist/apiAccess?scope=datasetMetadataSearch&q=文化&limit=3&offse
 */
public struct GetZooParameter: ParameterProtocol
{
...
public struct GetZooParser: WebAPIJsonParserProtocol
{
    public typealias TResult = GetZooModel
...
public struct GetZooModel: ResponseModelProtocol, JsonDeserializeable, JsonSerializeable, PropertyMapping {

    public typealias TParameter = GetZooParameter
    public var response: HTTPURLResponse?
    public var responseData: Data?
    public var parameter: GetZooParameter?
